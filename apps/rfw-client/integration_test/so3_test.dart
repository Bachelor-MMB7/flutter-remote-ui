...
// pump until the widget shows up (max ~15 s), pumpAndSettle would hang on the spinner
...
void main() {
  ...
  testWidgets('SO3 series', (tester) async {
    app.main();
    // wait for the initial fetch (tenant-a) to finish
    await pumpUntilFound(tester, find.text('Continue'));

    // switch to tenant-b, the workflow the baseline mirrors
    await tester.tap(find.text('Tenant A'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('Tenant B').last);
    // wait for the tenant-b fetch to finish
    await pumpUntilFound(tester, find.text('Continue'));
